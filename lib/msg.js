...
// View once types all 3 handle karanna
const VO_TYPES = ['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension']

const downloadMediaMessage = async(m, filename) => {
	// V1 + V2 + V2Ext all handle
	if (VO_TYPES.includes(m.type)) {
		m.type = m.msg.type
	}
...
const sms = (sock, m) => {
	...
	if (m.message) {
		...
		// V1 + V2 + V2Ext current message handle
		const isVO = VO_TYPES.includes(m.type)
		m.msg = isVO
		...
		if (m.msg) {
			...
			if (m.quoted) {
				...
				// V1 + V2 + V2Ext quoted message handle ← KEY FIX
				const isQVO = VO_TYPES.includes(m.quoted.type)
				m.quoted.msg = isQVO
